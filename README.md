...
Klickbares Mockup auf Basis des Design-Dokuments `public/design/Fortbildung UX.dc.html`
...
Deployment auf Cloudflare Workers: `npx wrangler deploy` (Konfiguration in `wrangler.jsonc`, ausgeliefert wird der Ordner `public/`).

## Aufbau

| Datei | Inhalt |
| --- | --- |
| `public/index.html` | Vergleichsansicht: Desktop und Mobile nebeneinander, Umschalter **Beide / Desktop / Mobile**, „Neu starten“ je Gerät. Über jedem Gerät steht, welcher Screen aus dem Design-Dokument gerade zu sehen ist (Link springt dorthin). |
| `public/app.html?device=desktop` / `app.html?device=mobile` | Die eigentliche App, einzeln aufrufbar. |
| `public/js/app.js`, `public/css/app.css` | Logik und Styles der App. |
| `public/design/` | Das originale Design-Dokument inkl. aller Beispiele und Skizzen (unverändert). |
